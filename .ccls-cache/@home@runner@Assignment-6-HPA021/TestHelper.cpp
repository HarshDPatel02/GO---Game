...
void TestHelper ::startup(const string &name, int compile_marks,
                          int total_marks) {
  assert(name != "");
...
bool TestHelper ::isCapturing() { return ptr_capture_old_puffer != NULL; }
...
void TestHelper ::printSummaryLine(const std::string &text, bool is_correct) {
  assert(text != "");
...
void TestHelper ::printSummaryLine(const std::string &text,
  ...
  if (achieved == maximum)
    ...
    cout << "Incorrect (";
  cout << right << setw(summary_max_digits) << achieved << " / " << right
       << setw(summary_max_digits) << maximum << ")" << endl;
}

void TestHelper ::printSummaryLine(const std::string &text, double percentage) {
  assert(text != "");
...
void TestHelper ::printMark(unsigned int achieved, unsigned int maximum) {
  assert(maximum >= 1);
  assert(achieved <= maximum);
